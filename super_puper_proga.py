...
class VideoDownloaderApp:
    def __init__(self, root):
        ...
        self.auto_replace_logo = tk.BooleanVar(value=False) # Автозамена лого после скачивания
    ...
    def setup_ui(self):
        ...
        # Фрейм для автозамены лого
        auto_logo_frame = ttk.Frame(self.root)
        auto_logo_frame.pack(fill="x", padx=10, pady=5)
        ttk.Checkbutton(auto_logo_frame, text="Автоматически заменять лого после скачивания всех эпизодов", 
                       variable=self.auto_replace_logo).pack(anchor="w")
    ...
    def download_series(self, url, series_index, total_series):
        """Скачивает одну серию и сразу конвертирует в MP4"""
        ...
        while consecutive_errors < max_consecutive_errors and counter < max_segments:
            ...
            if success:
                ...
                if error_type == "not_found":
                    ...
                    print(f"Сегмент {counter} - серверная ошибка (500+), пропускаем")
                    consecutive_errors += 1  # Пропускаем сегмент при 500+ ошибке
                else:
                    consecutive_errors += 1
            counter += 1
        ...
        if downloaded_segments > 0:
            ...
            if self.combine_segments(segments_dir, episode_num, temp_input):
                if self.convert_to_mp4(temp_input, output_name):
                    ...
                    print(f"Серия {episode_num} успешно скачана и сконвертирована: {output_name}")
                    return True
                else:
    ...
    def download_segment_with_retry(self, url, segment_num, episode_num, segments_dir, max_retries=5):
        """Скачивает сегмент с интеллектуальными повторами"""
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200 and len(response.content) > 1000:
                    ...
                    print(f"Сегмент {segment_num} - серверная ошибка {response.status_code}")
    ...
    def download_all_series(self):
        ...
        # Добавляем скачанные файлы в список
        ...
        # Если включена автозамена и есть скачанные файлы, запускаем обработку
        if successful_downloads > 0 and self.auto_replace_logo.get():
            self.root.after(1000, self.start_processing)  # Задержка 1 секунда перед началом обработки
    ...
    def process_videos(self):
        ...
        for i, input_file in enumerate(self.video_files):
            # Пропускаем файлы, которые уже имеют логотип в названии
            if "_logo_" in input_file:
                print(f"Пропускаем файл с логотипом: {os.path.basename(input_file)}")
